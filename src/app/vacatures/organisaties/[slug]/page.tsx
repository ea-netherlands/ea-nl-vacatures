...
export async function generateMetadata({
  ...
  if (!employer) return { title: 'Organisatie niet gevonden' }
  return bilingualMetadata({
    locale: 'nl',
    nlPath: ROUTES.nl.employer(slug),
    enPath: ROUTES.en.employer(slug),
    // "werken bij Adessium" is exactly the query this page should win (§9.4).
    title: `Werken bij ${employer.name}`,
    description: employer.leverageNoteNl?.slice(0, 155) ?? undefined,
  })
}

export default async function Page({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  return <EmployerPage locale="nl" slug={slug} />
}
